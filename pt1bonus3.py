...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 27 20:31:14 2018
...
#loop over classes
for clss in ["squares","triangles","rectangles"]:
    ...
    for i in range(nfigs):
        ...
        if clss == "squares":
            ...
            path_data= [
                (Path.MOVETO, (basex, basey)), #move to base position of this image
                (Path.LINETO, ((basex+length)*2, basey)), #draw line across to the right
                (Path.LINETO, (2*(basex+length), 2*(basey+length))), #draw line up
                (Path.LINETO, (basex, (basey+length)*2)), #draw line back across to the left
                (Path.LINETO, (basex, basey)), #draw line back down to base postiion
            ]
